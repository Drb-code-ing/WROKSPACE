# TS 类型之争与 Harness 择优工程：从 interface 与 type 的面试必考点，到让大模型自我评分的自动化流水线

上一节我们把"最重的活"——1.5B 大模型——搬进了浏览器，端侧 AI 的地基有了。但地基要想在面试里站得住，还得回到最基础的问题上：**类型、面试考点、工程化**。第五十五天笔记画风一转，从"搬模型"回到"打地基"，两条线并行：

- 一条是"类型"：**TypeScript 的 `interface` 与 `type` 之争**，前端面试的高频必考题，从五大差异一路聊到 React 组件里的"面向接口编程"；
- 一条是"工程"：**LLM Harness 工程**，用"让大模型当评委 + 并行生成择优"的流水线，去解决 LLM 的幻觉与落地问题。

这两条线看起来一个在编译期、一个在运行期，一个管"代码怎么约束"、一个管"结果怎么把关"，但串起来其实是同一件事：**用结构化的方式，减少不确定性**——类型把"变量的不确定性"在编译期锁死，Harness 把"生成结果的不确定性"在运行期筛掉。

还记得 V053 聊过的 "Harness 工作流 / 质量阀门" 吗？那是概念上的驾驭之道。这一天，它终于变成了一行行**可运行的代码**。
...
V053 我们聊过"Harness 工作流"——那是驾驭 AI 的理念：需求、设计、规范、Git 阀门，让高速产出不失控。这一天的 `harness` 更具体、更"跑得起来"：它把"生成结果怎么把关"本身变成了代码。核心是一套 **LLM as Judge + Best of N Sampling** 的组合模式。
...
- 而 Harness 工程，则是把 V053 的"驾驭理念"落成了可运行的代码：**Best of N 并行生成，LLM as Judge 自动打分，三阶段解耦择优**。
...
类型让代码"说什么就是什么"，Harness 让模型"选出来的就是最好的"。面试会考你前者，而工程会考验你后者——两个都拿下，才算真正从"会写 TS"走到"会驾驭 AI"。
